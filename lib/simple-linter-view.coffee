...
# Basic parsing tool with callback functionality
lint = (callback, cmd, params, regex) ->
# flake = (filePath, callback, cmd, params) ->
  ...
  proc.on 'exit', (exit_code, signal) ->
    if exit_code == 1 and errors.length == 0
      console.log('Linter crashed or is unavailable. Check command path.')
    callback errors


class SimpleLinterView extends SelectListView
  ...
  @activate: ->
    new SimpleLinterView

  activate: (state) ->
    atom.workspaceView.command 'simple-linter:toggle', => @toggle()
    atom.workspaceView.command 'core:save', =>
      on_save = atom.config.get "simple-linter.validateOnSave"
      if on_save == true
        @toggle()

  initialize: ->
    super
    @addClass('simple-linter overlay from-top')
